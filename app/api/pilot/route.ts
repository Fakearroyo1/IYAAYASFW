...
export async function GET(request:Request){try{const user=await getUser();if(!user)return json({error:'Sign in with your approved email and password.'},401);if(!env.DB)return json({error:'The shared store is temporarily unavailable. Please try again.'},503);if(!await rateLimit(env.DB,'read:'+user.memberId,120,60000))return json({error:'Too many requests. Try again in one minute.'},429);const q=new URL(request.url).searchParams;const response=json(await readState(env.DB,user,{view:q.get('view')||'account',section:q.get('section')||'overview',search:q.get('search')||'',filter:q.get('filter')||'all',includeAdmin:await adminVerified(env.DB,user)}));const cookie=await renewSession(env.DB,request.headers.get('cookie'));if(cookie)response.headers.set('Set-Cookie',cookie);return response}catch(e){if(e instanceof RequestError||e instanceof PilotError)return json({error:e.message},e.status);console.error('pilot read failed',e instanceof Error?e.name:'UnknownError');return json({error:'The store could not load. Your records have not been changed.'},503)}}
export async function POST(request:Request){try{
 ...
 if(!['order','payment'].includes(body.action))await requireAdminAccess(env.DB,user);
 const result=await mutate(env.DB,user,body);if('order' in result&&result.order&&!await adminVerified(env.DB,user)){const {cost,fingerprint,...order}=result.order;return json({...result,order})}if('payment' in result&&result.payment&&!await adminVerified(env.DB,user)){const {fingerprint,...payment}=result.payment;return json({...result,payment})}return json(result);
 }catch(e){if(e instanceof RequestError||e instanceof PilotError)return json({error:e.message},e.status);console.error('pilot write failed',e instanceof Error?e.name:'UnknownError');return json({error:'Could not confirm this action. Keep this page open and retry the same request.'},503)}}
