...
export async function GET(request: Request) {
  try {
    ...
    if (!user) throw new RequestError("Sign in to export records.", 401);
    await requireAdminAccess(db, user);
    if (!(await rateLimit(db, "export:" + user.memberId, 3, 900000)))
      throw new RequestError(
        "Export limit reached. Try again in 15 minutes.",
        429,
      );
    const member = await identity(db, user);
    if (!member)
        ...
        try {
          ...
          if(kinds[kind]==='workflow'){
            ...
            data.products = await catalogState(
              db,
              member,
              await accessFor(db, member as any),
            );
            finished = true;
          } else {
            const page = await historyPage(db, member, kinds[kind], {
              admin: true,
              limit: 40,
              cursor,
              from,
            ...
            finished = !cursor;
            data.admin[kinds[kind]] = page.records;
            if (kind === "items" && page.records.length) {
              const ids = [...new Set(page.records.map((r) => r.order_id))];
              data.admin.orders = await rows(
                db,
                `SELECT * FROM order_balances WHERE id IN(${ids.map(() => "?").join(",")})`,
                ...ids,
              );
            }
            if (kind !== "members") {
              const ids = [
                ...new Set(
                  [
                    ...
                    .flatMap((r: any) => [r.member_id, r.verified_by, r.actor])
                    .concat(data.admin.events.map((r: any) => r.actor))
                    .filter(Boolean),
                ),
              ];
              if (ids.length)
                data.admin.members = await rows(
                  db,
                  `SELECT id,name,email FROM members WHERE id IN(${ids.map(() => "?").join(",")})`,
                  ...ids,
                );
            }
          }
          ...
          if (finished) {
            if (!started)
              controller.enqueue(encoder.encode("\uFEFF" + toCsv([])));
            controller.close();
          }
        } catch (e) {
          controller.error(e);
    ...
    return new Response(stream, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="unit-supply-${kind}-${new Date().toISOString().slice(0, 10)}.csv"`,
