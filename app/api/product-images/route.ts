...
export async function POST(request:Request){
 try{
  ...
  const user=await getUser();const member=await identity(env.DB,user);if(member?.role!=='admin')return json('Administrator access is required.',403);
  ...
  const chunks:Uint8Array[]=[];let size=0,timedOut=false;const timeout=setTimeout(()=>{timedOut=true;void reader.cancel()},10000);
  try{
  while(true){const {done,value}=await reader.read();if(done)break;size+=value.byteLength;if(size>6*1024*1024){await reader.cancel();return json('Choose an image smaller than 5 MB.',413)}chunks.push(value)}if(timedOut)throw new RequestError('Upload timed out. Try again.',408);
 ...
 }catch(e){if(e instanceof RequestError)return json(e.message,e.status);console.error('Image upload failed',e instanceof Error?e.name:'UnknownError');return json('The image could not upload. Your product has not changed. Try again.',503)}
}
