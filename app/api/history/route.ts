...
export async function GET(request:Request){try{
 ...
 const options={admin,cursor:q.get('cursor')||'',from:q.get('from')||'',to:q.get('to')||'',search:q.get('search')||'',filter:q.get('filter')||'',productId:q.get('productId')||'',memberId:q.get('memberId')||''};
