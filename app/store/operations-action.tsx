...
export function useOperations(onSuccess:()=>Promise<unknown>,endpoint="/api/operations"){
 ...
 async function send(body:Row,retry=false):Promise<Row|null>{
  ...
  try{
   const r=await secureFetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request),signal:controller.signal}),j=await r.json() as Row;
...
export async function operationsRead(q:Record<string,string>){const r=await secureFetch('/api/operations?'+new URLSearchParams(q),{cache:'no-store'}),j=await r.json() as Row;if(!r.ok)throw Error(j.error||'Could not load this view.');return j as Row}

export async function roadmapRead(q:Record<string,string>){const r=await secureFetch('/api/roadmap?'+new URLSearchParams(q),{cache:'no-store'}),j=await r.json() as Row;if(!r.ok)throw Error(j.error||'Could not load this view.');return j;}
