 ...
 return <Container className={embedded?'identity-embedded':'identity-shell'}><section className="panel identity-panel">
  ...
  {!context?<p>Loading…</p>:!context.enabled?<><p>This update is not enabled yet. Your existing password still works.</p><a href="/login?password=1">Use existing password</a></>:<>
   ...
   {account&&context.host==='member'&&<>
    <section className="identity-block"><h2>Linked methods</h2>{account.hasPassword&&<p>Existing password · available</p>}{account.methods.map((m:Row)=><div className="identity-row" key={m.id}><span><strong>{m.label}</strong><small>{names[m.kind as Method]} · {m.status}{m.observed_email?' · '+m.observed_email:''}</small></span>{m.status==='active'&&<div>{m.kind==='passkey'&&<Button variant="ghost" disabled={busy} onClick={()=>{const label=window.prompt('Passkey name',m.label);if(label)void run(async()=>{await api({action:'own',operation:'rename',target:m.id,label});await refreshAccount();});}}>Rename</Button>}<Button variant="ghost" disabled={busy} onClick={()=>run(()=>start('unlink:'+m.id))}>Remove</Button></div>}</div>)}<p>{context.rollout?.eligible===false?'New sign-in methods are currently restricted for this account. Your existing password still works.':'Adding a backup method is recommended. You can keep using your existing password.'}</p><div className="identity-choices">{(Object.keys(names) as Method[]).map(m=><Button key={m} variant="outline" disabled={busy||!context.methods[m]||context.rollout?.eligible===false} onClick={()=>run(()=>start('add:'+m))}>Add {names[m]}</Button>)}</div></section>
