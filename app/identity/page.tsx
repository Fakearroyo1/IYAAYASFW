...
export default function IdentityPage({embedded=false}:{embedded?:boolean}={}){
 ...
 return <Container className={embedded?'identity-embedded':'identity-shell'}><section className="panel identity-panel">
  ...
  {!context?<p>Loading…</p>:!context.enabled?<><p>This update is not enabled yet. Your existing password still works.</p><a href="/login?password=1">Use existing password</a></>:<>
   ...
   {context.host==='admin'&&!context.user&&<><p>Complete sign-in with your approved administrator account.</p><Button disabled={busy} onClick={()=>run(async()=>follow(await api({action:'adminLogin',section:new URLSearchParams(window.location.search).get('section')})))}>Enter management</Button></>}
   ...
   {context.owner&&admin&&<section className="identity-block"><h2>Accounts &amp; sign-in</h2><p>Manage sign-in methods under each existing member ID. Invitations and verified login requests add a method to that member without moving purchase history or balances.</p>
    ...
    {workspace==='import'&&<>
    <h3>CSV import</h3><p>Upload or paste up to 100 rows using existing member IDs. Preview exact changes before applying selected rows. Blank cells preserve values; an email alone never chooses a member. Provider-address columns create first-login grants, not links to an already verified provider identity.</p>
    <Button variant="outline" disabled={busy} onClick={()=>run(async()=>{const template=await api({action:'importTemplate'});setCsv(template.csv);setPreview(null);setSelectedRows([]);})}>Use current member IDs as template</Button>
    <label className="field">Upload CSV file<Input type="file" accept=".csv,text/csv" disabled={busy} onChange={e=>{const file=e.currentTarget.files?.[0];e.currentTarget.value='';if(file)void run(async()=>{if(file.size>65536)throw Error('Choose a CSV file smaller than 64 KB.');setCsv(await file.text());setPreview(null);setSelectedRows([]);});}}/></label>
    <code className="identity-template">member_id,display_name,contact_email,google_bootstrap_email,microsoft_bootstrap_email,access_enabled</code><label className="field">CSV rows<textarea rows={6} value={csv} onChange={e=>{setCsv(e.target.value);setPreview(null);setSelectedRows([]);}} maxLength={65536}/></label><Button disabled={busy||!csv} variant="outline" onClick={()=>run(async()=>{const p=await api({action:'importPreview',csv});setPreview(p);setSelectedRows(p.rows.filter((r:Row)=>r.status==='Update').map((r:Row)=>r.number));})}>Preview changes</Button>
