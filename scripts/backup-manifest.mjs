...
export function databaseManifest(db){
 ...
 return {schemaHash:sha256(serialize(schema)),schema,objects,memberStateHash:sha256(serialize(members)),financialTotals:db.prepare('SELECT count(*) members,coalesce(sum(debt),0) debt,coalesce(sum(credit),0) credit FROM members').get()};
}
