...
export default {
 async scheduled(_event:ScheduledController,env:{DB:D1Database},ctx:ExecutionContext){ctx.waitUntil(cleanExpiredSecurityRecords(env.DB))},
 async fetch(request:Request,env:unknown,ctx:ExecutionContext):Promise<Response>{
  const nonce=Array.from(crypto.getRandomValues(new Uint8Array(16)),n=>n.toString(16).padStart(2,'0')).join('');
  const csp=["default-src 'self'",`script-src 'self' 'nonce-${nonce}' 'strict-dynamic' https://challenges.cloudflare.com`,"style-src 'self' 'unsafe-inline'","img-src 'self' data: blob:","font-src 'self'","connect-src 'self' https://challenges.cloudflare.com","frame-src https://challenges.cloudflare.com","object-src 'none'","base-uri 'none'","form-action 'self'","frame-ancestors 'none'"].join('; ');
...
async function cleanExpiredSecurityRecords(db:D1Database){
 const now=Date.now();
 for(const [table,key] of [['auth_limits','id'],['auth_admin_access','token_hash'],['auth_setup','member_id'],['auth_recovery','member_id']]){
  await db.prepare(`DELETE FROM ${table} WHERE ${key} IN (SELECT ${key} FROM ${table} WHERE expires_at<? LIMIT 500)`).bind(now).run();
 }
 await db.prepare("DELETE FROM auth_sessions WHERE token_hash IN (SELECT s.token_hash FROM auth_sessions s JOIN members m ON m.id=s.member_id WHERE s.expires_at<? OR s.created_at<?-CASE WHEN m.role='admin' THEN 43200000 ELSE 2592000000 END LIMIT 500)").bind(now,now).run();
}
