...
export async function batchAtomic(db:DB,statements:D1PreparedStatement[]){try{return await db.batch([...statements,stmt(db,'DELETE FROM guards')])}catch(e){if(/Reopen the accounting month/.test(String(e)))fail('Reopen the original accounting month before changing this financial record.',409);if(/constraint|unique/i.test(String(e)))fail('The record changed or this action was already recorded. Refresh and check before trying again.',409);throw e}}

// Revalidate the actual session inside a money/stock write transaction.
export function sessionGuard(db:DB,memberId:string,tokenHash?:string,admin=false){return tokenHash?[guard(db,"EXISTS(SELECT 1 FROM auth_sessions s JOIN members m ON m.id=s.member_id WHERE s.token_hash=? AND m.id=? AND m.active=1 AND s.expires_at>? AND s.created_at>?-CASE WHEN m.role='admin' THEN 43200000 ELSE 2592000000 END)",tokenHash,memberId,Date.now(),Date.now()),...(admin?[guard(db,"EXISTS(SELECT 1 FROM auth_admin_access WHERE token_hash=? AND expires_at>?)",tokenHash,Date.now())]:[])]:[]}
